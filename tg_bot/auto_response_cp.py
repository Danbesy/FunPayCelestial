...
def init_auto_response_cp(celestial: Celestial, *args):
    tg = celestial.telegram
    bot = tg.bot
    ...
    def check_command_exists(command_index: int, message_obj: Message, reply_mode: bool = True) -> bool:
        ...
        """
        if command_index > len(celestial.RAW_AR_CFG.sections()) - 1:
    ...
    def open_commands_list(c: CallbackQuery):
        ...
        bot.edit_message_text(_("desc_ar_list"), c.message.chat.id, c.message.id,
                              reply_markup=keyboards.commands_list(celestial, offset))
    ...
    def add_command(m: Message):
        ...
        for cmd in commands:
            if commands.count(cmd) > 1:
                ...
                return
            if cmd in celestial.AR_CFG.sections():
                bot.reply_to(m, _("ar_cmd_already_exists_err", utils.escape(cmd)), reply_markup=error_keyboard)
                return

        celestial.RAW_AR_CFG.add_section(raw_command)
        celestial.RAW_AR_CFG.set(raw_command, "response", "Данной команде необходимо настроить текст ответа :(")
        celestial.RAW_AR_CFG.set(raw_command, "telegramNotification", "0")

        for cmd in commands:
            celestial.AR_CFG.add_section(cmd)
            celestial.AR_CFG.set(cmd, "response", "Данной команде необходимо настроить текст ответа :(")
            celestial.AR_CFG.set(cmd, "telegramNotification", "0")

        celestial.save_config(celestial.RAW_AR_CFG, "configs/auto_response.cfg")

        command_index = len(celestial.RAW_AR_CFG.sections()) - 1
    ...
    def open_edit_command_cp(c: CallbackQuery):
        ...
        keyboard = keyboards.edit_command(celestial, command_index, offset)

        command = celestial.RAW_AR_CFG.sections()[command_index]
        command_obj = celestial.RAW_AR_CFG[command]
...
<i>{_('gl_last_update')}:</i>  <code>{datetime.datetime.now().strftime('%H:%M:%S')}</code>"""
    ...
    def edit_command_response(m: Message):
        ...
        celestial.RAW_AR_CFG.set(command, "response", response_text)
        for cmd in commands:
            celestial.AR_CFG.set(cmd, "response", response_text)
        celestial.save_config(celestial.RAW_AR_CFG, "configs/auto_response.cfg")
    ...
    def edit_command_notification(m: Message):
        ...
        command = celestial.RAW_AR_CFG.sections()[command_index]
        commands = [i.strip() for i in command.split("|") if i.strip()]
        celestial.RAW_AR_CFG.set(command, "notificationText", notification_text)

        for cmd in commands:
            celestial.AR_CFG.set(cmd, "notificationText", notification_text)
        celestial.save_config(celestial.RAW_AR_CFG, "configs/auto_response.cfg")
    ...
    def switch_notification(c: CallbackQuery):
        ...
        if command_obj.get("telegramNotification") in [None, "0"]:
            ...
            value = "0"
        celestial.RAW_AR_CFG.set(command, "telegramNotification", value)
        for cmd in commands:
            celestial.AR_CFG.set(cmd, "telegramNotification", value)
        celestial.save_config(celestial.RAW_AR_CFG, "configs/auto_response.cfg")
    ...
    def del_command(c: CallbackQuery):
        ...
        command = celestial.RAW_AR_CFG.sections()[command_index]
        commands = [i.strip() for i in command.split("|") if i.strip()]
        celestial.RAW_AR_CFG.remove_section(command)
        for cmd in commands:
            celestial.AR_CFG.remove_section(cmd)
        celestial.save_config(celestial.RAW_AR_CFG, "configs/auto_response.cfg")
