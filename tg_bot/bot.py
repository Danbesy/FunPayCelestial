...
from Utils import celestial_tools, updater
...
class TGBot:
    def __init__(self, celestial: Celestial):
        self.celestial = celestial
        self.bot = telebot.TeleBot(self.celestial.MAIN_CFG["Telegram"]["token"], parse_mode="HTML",
    ...
    def reg_admin(self, m: Message):
        ...
        if m.chat.type != "private" or (m.from_user.id in self.attempts and self.attempts.get(m.from_user.id) >= 5):
            return
        if m.text == self.celestial.MAIN_CFG["Telegram"]["secretKey"]:
    ...
    def send_settings_menu(self, m: Message):
        ...
        """
        self.bot.send_message(m.chat.id, _("desc_main"), reply_markup=kb.settings_sections(self.celestial))
    ...
    def send_profile(self, m: Message):
        ...
        try:
            self.celestial.account.get()
            self.celestial.balance = self.celestial.get_balance()
            self.bot.send_message(m.chat.id, utils.generate_profile_text(self.celestial),
    ...
    def update_profile(self, c: CallbackQuery):
        ...
        try:
            self.celestial.account.get()
            self.celestial.balance = self.celestial.get_balance()
            self.bot.edit_message_text(utils.generate_profile_text(self.celestial), c.message.chat.id,
                                c.message.id,
                                reply_markup=telebot.types.InlineKeyboardMarkup()
                                .add(telebot.types.InlineKeyboardButton("🔄 Обновить", callback_data="update_profile"))
                                .add(telebot.types.InlineKeyboardButton("▶️ Еще", callback_data="update_adv_profile"))
    ...
    def update_adv_profile(self, c: CallbackQuery):
        ...
        try:
            self.celestial.account.get()
            self.celestial.balance = self.celestial.get_balance()
            self.bot.edit_message_text(utils.generate_adv_profile(self.celestial.account), c.message.chat.id,
                                c.message.id,
                                reply_markup=telebot.types.InlineKeyboardMarkup()
    ...
    def send_orders(self, m: telebot.types.Message):
        ...
        try:
            orders = utils.get_all_old_orders(self.celestial.account)
        except:
    ...
    def manual_delivery_text(self, m: Message):
        ...
        self.celestial.delivery_tests[key] = lot_name
    ...
    def ban(self, m: Message):
        ...
        if nickname in self.celestial.blacklist:
            self.bot.send_message(m.chat.id, _("already_blacklisted", nickname))
            return

        self.celestial.blacklist.append(nickname)
        celestial_tools.cache_blacklist(self.celestial.blacklist)
    ...
    def unban(self, m: Message):
        ...
        self.clear_state(m.chat.id, m.from_user.id, True)
        nickname = m.text.strip()
        if nickname not in self.celestial.blacklist:
            self.bot.send_message(m.chat.id, _("not_blacklisted", nickname))
            return
        self.celestial.blacklist.remove(nickname)
        celestial_tools.cache_blacklist(self.celestial.blacklist)
    ...
    def send_ban_list(self, m: Message):
        ...
        Отправляет ЧС.
        """
        if not self.celestial.blacklist:
            self.bot.send_message(m.chat.id, _("blacklist_empty"))
            return
        blacklist = ", ".join(f"<code>{i}</code>" for i in self.celestial.blacklist)
    ...
    def edit_watermark(self, m: Message):
        ...
        self.celestial.MAIN_CFG["Other"]["watermark"] = watermark
        self.celestial.save_config(self.celestial.MAIN_CFG, "configs/_main.cfg")
        if watermark:
    ...
    def about(self, m: Message):
        ...
        """
        self.bot.send_message(m.chat.id, _("about", self.celestial.VERSION))

    def check_updates(self, m: Message):
        curr_tag = f"v{self.celestial.VERSION}"
    ...
    def send_system_info(self, m: Message):
        ...
        uptime = current_time - self.celestial.start_time
        ...
        self.bot.send_message(m.chat.id, _("sys_info", cpu_usage, psutil.Process().cpu_percent(),
                                           ...
                                           celestial_tools.time_to_str(uptime), m.chat.id))

    def restart_celestial(self, m: Message):
        """
        Перезапускает FunPayCelestial.
        """
        self.bot.send_message(m.chat.id, _("restarting"))
        celestial_tools.restart_program()

    def ask_power_off(self, m: Message):
        """
        Просит подтверждение на отключение FPC.
        """
        self.bot.send_message(m.chat.id, _("power_off_0"), reply_markup=kb.power_off(self.celestial.instance_id, 0))
    ...
    def power_off(self, c: CallbackQuery):
        ...
        if instance_id != self.celestial.instance_id:
    ...
    def send_funpay_message(self, message: Message):
        ...
        result = self.celestial.send_message(node_id, response_text, username)
        if result:
    ...
    def edit_greetings_text(self, m: Message):
        self.clear_state(m.chat.id, m.from_user.id, True)
        self.celestial.MAIN_CFG["Greetings"]["greetingsText"] = m.text
    ...
    def edit_order_confirm_reply_text(self, m: Message):
        self.clear_state(m.chat.id, m.from_user.id, True)
        self.celestial.MAIN_CFG["OrderConfirm"]["replyText"] = m.text
    ...
    def edit_review_reply_text(self, m: Message):
        ...
        self.clear_state(m.chat.id, m.from_user.id, True)
        self.celestial.MAIN_CFG["ReviewReply"][f"star{stars}ReplyText"] = m.text
    ...
    def extend_new_message_notification(self, c: CallbackQuery):
        ...
        try:
            chat = self.celestial.account.get_chat(int(chat_id))
        except:
            self.bot.answer_callback_query(c.id)
        ...
        for i in messages:
            if i.author_id == last_message_author_id:
                author = ""
            elif i.author_id == self.celestial.account.id:
    ...
    def refund(self, c: CallbackQuery):
        ...
        while attempts:
            try:
                self.celestial.account.refund(order_id)
                break
            except:
                if not new_msg:
    ...
    def open_cp(self, c: CallbackQuery):
        ...
        self.bot.edit_message_text(_("desc_main"), c.message.chat.id, c.message.id,
                                   reply_markup=kb.settings_sections(self.celestial))
        self.bot.answer_callback_query(c.id)
    ...
    def switch_param(self, c: CallbackQuery):
        ...
        if section == "FunPay" and option == "oldMsgGetMode":
            self.celestial.switch_msg_get_mode()
        else:
            self.celestial.MAIN_CFG[section][option] = str(int(not int(self.celestial.MAIN_CFG[section][option])))
            self.celestial.save_config(self.celestial.MAIN_CFG, "configs/_main.cfg")
        ...
        self.bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                           reply_markup=sections[section](self.celestial))
        logger.info(_("log_param_changed", c.from_user.username, c.from_user.id, option, section, self.celestial.MAIN_CFG[section][option]))
        self.bot.answer_callback_query(c.id)
    ...
    def switch_chat_notification(self, c: CallbackQuery):
        ...
        self.bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                           reply_markup=keyboard(self.celestial, c.message.chat.id))
        self.bot.answer_callback_query(c.id)
    ...
    def open_settings_section(self, c: CallbackQuery):
        ...
        sections = {
            "main": (_("desc_gs"), kb.main_settings, [self.celestial]),
            "tg": (_("desc_ns", c.message.chat.id), kb.notifications_settings, [self.celestial, c.message.chat.id]),
            "bl": (_("desc_bl"), kb.blacklist_settings, [self.celestial]),
            "ar": (_("desc_ar"), skb.AR_SETTINGS, []),
            "ad": (_("desc_ad"), skb.AD_SETTINGS, []),
            "mv": (_("desc_mv"), kb.new_message_view_settings, [self.celestial]),
            "rr": (_("desc_or"), kb.review_reply_settings, [self.celestial]),
            "gr": (_("desc_gr", utils.escape(self.celestial.MAIN_CFG['Greetings']['greetingsText'])),
                   kb.greeting_settings, [self.celestial]),
            "oc": (_("desc_oc", utils.escape(self.celestial.MAIN_CFG['OrderConfirm']['replyText'])),
                   kb.order_confirm_reply_settings, [self.celestial])
        }
    ...
    def send_announcements_kb(self, m: Message):
        ...
        """
        self.bot.send_message(m.chat.id, _("desc_an"), reply_markup=kb.announcements_settings(self.celestial, m.chat.id))

    def send_review_reply_text(self, c: CallbackQuery):
        stars = int(c.data.split(":")[1])
        text = self.celestial.MAIN_CFG["ReviewReply"][f"star{stars}ReplyText"]
        keyboard = K() \
            .row(B(_("gl_back"), callback_data=f"{CBT.CATEGORY}:rr"),
                 B(_("gl_edit"), callback_data=f"{CBT.EDIT_REVIEW_REPLY_TEXT}:{stars}"))
        if not text:
            ...
            self.bot.send_message(c.message.chat.id, _("review_reply_text", "⭐"*stars,
                                                       self.celestial.MAIN_CFG['ReviewReply'][f'star{stars}ReplyText']),
    ...
    def switch_lang(self, c: CallbackQuery):
        ...
        self.celestial.MAIN_CFG["Other"]["language"] = lang
        self.celestial.save_config(self.celestial.MAIN_CFG, "configs/_main.cfg")
        if localizer.current_language == "eng":
            self.bot.answer_callback_query(c.id, "(⁠≧⁠▽⁠≦⁠)", show_alert=True)
        self.open_cp(c)
    ...
    def __register_handlers(self):
        ...
        self.msg_handler(self.restart_celestial, commands=["restart"])
