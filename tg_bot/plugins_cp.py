...
def init_plugins_cp(celestial: Celestial, *args):
    ...
    def check_plugin_exists(uuid: str, message_obj: Message) -> bool:
        ...
        """
        if uuid not in celestial.plugins:
    ...
    def open_plugins_list(c: CallbackQuery):
        ...
        bot.edit_message_text(_("desc_pl"), c.message.chat.id, c.message.id,
                              reply_markup=keyboards.plugins_list(celestial, offset))
    ...
    def open_edit_plugin_cp(c: CallbackQuery):
        ...
        plugin_data = celestial.plugins[uuid]
...
<i>{_('gl_last_update')}:</i>  <code>{datetime.datetime.now().strftime('%H:%M:%S')}</code>"""
        keyboard = keyboards.edit_plugin(celestial, uuid, offset)
...
{commands_text}"""
    ...
    def toggle_plugin(c: CallbackQuery):
        ...
        celestial.toggle_plugin(uuid)
        c.data = f"{CBT.EDIT_PLUGIN}:{uuid}:{offset}"
        logger.info(_("log_pl_activated" if celestial.plugins[uuid].enabled else "log_pl_deactivated",
                      c.from_user.username, c.from_user.id, celestial.plugins[uuid].name))
    ...
    def ask_delete_plugin(c: CallbackQuery):
        ...
        bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                      reply_markup=keyboards.edit_plugin(celestial, uuid, offset, True))
        bot.answer_callback_query(c.id)
    ...
    def cancel_delete_plugin(c: CallbackQuery):
        ...
        bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                      reply_markup=keyboards.edit_plugin(celestial, uuid, offset))
        bot.answer_callback_query(c.id)
    ...
    def delete_plugin(c: CallbackQuery):
        ...
        if not os.path.exists(celestial.plugins[uuid].path):
            bot.answer_callback_query(c.id, _("pl_file_not_found_err", utils.escape(celestial.plugins[uuid].path)),
                                      show_alert=True)
            return

        if celestial.plugins[uuid].delete_handler:
            try:
                celestial.plugins[uuid].delete_handler(celestial, c)
            except:
                logger.error(_("log_pl_delete_handler_err", celestial.plugins[uuid].name))
                logger.debug("TRACEBACK", exc_info=True)

        os.remove(celestial.plugins[uuid].path)
        logger.info(_("log_pl_deleted", c.from_user.username, c.from_user.id, celestial.plugins[uuid].name))
        celestial.plugins.pop(uuid)
