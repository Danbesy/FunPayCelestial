...
def settings_sections(c: Celestial) -> K:
...
def new_message_view_settings(c: Celestial) -> K:
    """
...
def greeting_settings(c: Celestial):
    """
...
def order_confirm_reply_settings(c: Celestial):
    """
...
def review_reply_settings(c: Celestial):
    """
...
def notifications_settings(c: Celestial, chat_id: int) -> K:
    """
...
def announcements_settings(c: Celestial, chat_id: int):
    """
...
def blacklist_settings(c: Celestial) -> K:
    """
...
def commands_list(c: Celestial, offset: int) -> K:
    """
...
def edit_command(c: Celestial, command_index: int, offset: int) -> K:
    """
...
def products_files_list(offset: int) -> K:
    ...
    for index, name in enumerate(files):
        amount = Utils.celestial_tools.count_products(f"storage/products/{name}")
...
def lots_list(celestial: Celestial, offset: int) -> K:
    """
    Создает клавиатуру со списком лотов с автовыдачей. (lots:<offset>).

    :param celestial: объект FunPayCelestial.
    :param offset: смещение списка лотов.

    :return: объект клавиатуры со списком лотов с автовыдачей.
    """
    keyboard = K()
    lots = celestial.AD_CFG.sections()[offset: offset + MENU_CFG.AD_BTNS_AMOUNT]
    ...
    keyboard = add_navigation_buttons(keyboard, offset, MENU_CFG.AD_BTNS_AMOUNT, len(lots),
                                      len(celestial.AD_CFG.sections()), CBT.AD_LOTS_LIST)
...
def funpay_lots_list(c: Celestial, offset: int):
    """
...
def edit_lot(c: Celestial, lot_number: int, offset: int) -> K:
    """
    Генерирует клавиатуру изменения лота (CBT.EDIT_AD_LOT:<lot_num>:<offset>).

    :param c: экземпляр FunPayCelestial.
...
def templates_list(c: Celestial, offset: int) -> K:
    """
...
def edit_template(c: Celestial, template_index: int, offset: int) -> K:
    """
...
def templates_list_ans_mode(c: Celestial, offset: int, node_id: int, username: str, prev_page: int,
...
def plugins_list(c: Celestial, offset: int):
    """
...
def edit_plugin(c: Celestial, uuid: str, offset: int, ask_to_delete: bool = False):
    """
