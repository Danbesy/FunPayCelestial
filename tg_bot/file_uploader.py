...
from Utils import config_loader as cfg_loader, exceptions as excs, celestial_tools
...
def init_uploader(celestial: Celestial):
    ...
    def upload_products_file(m: types.Message):
        ...
        try:
            products_count = celestial_tools.count_products(f"storage/products/{utils.escape(m.document.file_name)}")
        except:
    ...
    def upload_main_config(m: types.Message):
        ...
        celestial.save_config(new_config, "configs/_main.cfg")
        logger.info(f"Пользователь $MAGENTA@{m.from_user.username} (id: {m.from_user.id})$RESET "
    ...
    def upload_auto_response_config(m: types.Message):
        ...
        celestial.RAW_AR_CFG, celestial.AR_CFG = raw_new_config, new_config
    ...
    def upload_auto_delivery_config(m: types.Message):
        ...
        celestial.AD_CFG = new_config
        celestial.save_config(celestial.AD_CFG, "configs/auto_delivery.cfg")
    ...
    def send_funpay_image(m: types.Message):
        ...
        try:
            ...
            file = tg.bot.download_file(file_info.file_path)
            image_id = celestial.account.upload_image(file)
            result = celestial.account.send_message(chat_id, None, username, image_id)
            if not result:
