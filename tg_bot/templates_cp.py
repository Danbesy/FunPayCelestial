...
def init_templates_cp(celestial: Celestial, *args):
    ...
    def check_template_exists(template_index: int, message_obj: Message) -> bool:
        ...
        """
        if template_index > len(celestial.telegram.answer_templates) - 1:
    ...
    def open_templates_list(c: CallbackQuery):
        ...
        bot.edit_message_text(_("desc_tmplt"), c.message.chat.id, c.message.id,
                              reply_markup=keyboards.templates_list(celestial, offset))
    ...
    def open_templates_list_in_ans_mode(c: CallbackQuery):
        ...
        bot.edit_message_reply_markup(c.message.chat.id, c.message.id,
                                      reply_markup=keyboards.templates_list_ans_mode(celestial, offset, node_id,
    ...
    def open_edit_template_cp(c: CallbackQuery):
        ...
        keyboard = keyboards.edit_template(celestial, template_index, offset)
        template = celestial.telegram.answer_templates[template_index]
    ...
    def send_template(c: CallbackQuery):
        ...
        result = celestial.send_message(node_id, text, username)
        if result:
