...
from Utils import celestial_tools
...
# INIT MESSAGE
def save_init_chats_handler(c: Celestial, e: InitialChatEvent):
    """
    ...
    if c.MAIN_CFG["Greetings"].getboolean("cacheInitChats") and e.chat.id not in c.old_users:
        ...
        celestial_tools.cache_old_users(c.old_users)


# NEW MESSAGE / LAST CHAT MESSAGE CHANGED
def old_log_msg_handler(c: Celestial, e: LastChatMessageChangedEvent):
    """
...
def log_msg_handler(c: Celestial, e: NewMessageEvent):
...
def greetings_handler(c: Celestial, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    text = celestial_tools.format_msg_text(c.MAIN_CFG["Greetings"]["greetingsText"], obj)
    Thread(target=c.send_message, args=(chat_id, text, chat_name), daemon=True).start()


def add_old_user_handler(c: Celestial, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
...
def send_response_handler(c: Celestial, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    response_text = celestial_tools.format_msg_text(c.AR_CFG[command]["response"], obj)
    Thread(target=c.send_message, args=(chat_id, response_text, chat_name), daemon=True).start()


def old_send_new_msg_notification_handler(c: Celestial, e: LastChatMessageChangedEvent):
...
def send_new_msg_notification_handler(c: Celestial, e: NewMessageEvent) -> None:
    """
...
def send_review_notification(c: Celestial, order: Order, chat_id: int, reply_text: str | None):
    if not c.telegram:
        return
...
def process_review_handler(c: Celestial, e: NewMessageEvent | LastChatMessageChangedEvent):
    if not c.old_mode_enabled:
        if isinstance(e, LastChatMessageChangedEvent):
            return
    ...
    def send_reply():
        ...
        if c.MAIN_CFG["ReviewReply"].getboolean(toggle) and c.MAIN_CFG["ReviewReply"].get(text):
            try:
                reply_text = celestial_tools.format_order_text(c.MAIN_CFG["ReviewReply"].get(text), order)
...
def send_command_notification_handler(c: Celestial, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    if not c.AR_CFG[command].get("notificationText"):
        ...
        text = celestial_tools.format_msg_text(c.AR_CFG[command]["notificationText"], obj)
...
def test_auto_delivery_handler(c: Celestial, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
...
def send_categories_raised_notification_handler(c: Celestial, cat: types.Category) -> None:
    """
...
def get_lot_config_by_name(c: Celestial, name: str) -> configparser.SectionProxy | None:
    """
    Ищет секцию лота в конфиге автовыдачи.

    :param c: объект FunPayCelestial'a.
...
def check_products_amount(config_obj: configparser.SectionProxy) -> int:
    ...
    if not file_name:
        return 1
    return celestial_tools.count_products(f"storage/products/{file_name}")


def update_current_lots_handler(c: Celestial, e: OrdersListChangedEvent):
...
def log_new_order_handler(c: Celestial, e: NewOrderEvent, *args):
    """
...
def setup_event_attributes_handler(c: Celestial, e: NewOrderEvent, *args):
...
def send_new_order_notification_handler(c: Celestial, e: NewOrderEvent, *args):
    """
...
def deliver_goods(c: Celestial, e: NewOrderEvent, *args):
    chat_id = c.account.get_chat_by_name(e.order.buyer_username).id
    cfg_obj = getattr(e, "config_section_obj")
    delivery_text = celestial_tools.format_order_text(cfg_obj["response"], e.order)
    ...
    try:
        if file_name := cfg_obj.get("productsFileName"):
            ...
            products, goods_left = celestial_tools.get_products(f"storage/products/{file_name}", amount)
            print(products)
    ...
    if not result:
        ...
        if file_name:
            celestial_tools.add_products(f"storage/products/{file_name}", products[0])
    else:
...
def deliver_product_handler(c: Celestial, e: NewOrderEvent, *args) -> None:
    """
...
def send_delivery_notification_handler(c: Celestial, e: NewOrderEvent):
    """
...
def update_lot_state(celestial: Celestial, lot: types.LotShortcut, task: int) -> bool:
    """
    Обновляет состояние лота

    :param celestial: объект Celestial'a.
    ...
    while attempts:
        try:
            lot_fields = celestial.account.get_lot_fields(lot.id)
            if task == 1:
                lot_fields.active = True
                celestial.account.save_lot(lot_fields)
...
def update_lots_states(celestial: Celestial, event: NewOrderEvent):
    if not any([celestial.autorestore_enabled, celestial.autodisable_enabled]):
        return
    if celestial.curr_profile_last_tag != event.runner_tag or celestial.last_state_change_tag == event.runner_tag:
        return

    lots = celestial.curr_profile.get_sorted_lots(1)

    deactivated = []
    restored = []
    for lot in celestial.profile.get_lots():
        # -1 - деактивировать
        # 0 - ничего не делать
        # 1 - восстановить
        current_task = 0
        config_obj = get_lot_config_by_name(celestial, lot.description)
        ...
        # Если же лот активен
            ...
            if config_obj:
                ...
                if all((not products_count, celestial.MAIN_CFG["FunPay"].getboolean("autoDisable"),
                        config_obj.get("disableAutoDisable") in ["0", None])):
                    current_task = -1

        if current_task:
            result = update_lot_state(celestial, lot, current_task)
            if result:
                if current_task == -1:
...
<code>{lots}</code>"""
        Thread(target=celestial.telegram.send_notification, args=(text, ),
...
<code>{lots}</code>"""
    ...
    celestial.last_state_change_tag = event.runner_tag


def update_lots_state_handler(celestial: Celestial, event: NewOrderEvent, *args):
    Thread(target=update_lots_states, args=(celestial, event), daemon=True).start()


# BIND_TO_ORDER_STATUS_CHANGED
def send_thank_u_message_handler(c: Celestial, e: OrderStatusChangedEvent):
    """
    ...
    text = celestial_tools.format_order_text(c.MAIN_CFG["OrderConfirm"]["replyText"], e.order)
...
def send_order_confirmed_notification_handler(celestial: Celestial, event: OrderStatusChangedEvent):
    """
    ...
    chat = celestial.account.get_chat_by_name(event.order.buyer_username, True)
    Thread(target=celestial.telegram.send_notification,
...
def send_bot_started_notification_handler(c: Celestial, *args):
    """
