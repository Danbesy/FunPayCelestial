...
def first_setup():
    ...
    print(f"{Fore.WHITE}{Style.BRIGHT}Привет! {Fore.MAGENTA}(˘ ³⁠ ˘⁠){Style.RESET_ALL}")
    time.sleep(sleep_time)

    print(f"\n{Fore.WHITE}{Style.BRIGHT}Не могу найти основной конфиг.. {Fore.MAGENTA}(⁠ﾉ⁠*⁠0⁠*⁠)⁠ﾉ{Style.RESET_ALL}")
    time.sleep(sleep_time)

    print(f"\n{Fore.WHITE}{Style.BRIGHT}Давай-ка проведём первичную настройку! {Fore.MAGENTA}(⁠•⁠ө⁠•⁠)⁠♡{Style.RESET_ALL}")
    time.sleep(sleep_time)

    while True:
        print(f"\n{Fore.MAGENTA}{Style.BRIGHT}┌── {Fore.WHITE}"
              f"Для начала введи токен (golden_key) твоего FunPay аккаунта {Fore.MAGENTA}(⁠♡⁠ω⁠♡){Style.RESET_ALL}")
        golden_key = input(f"{Fore.MAGENTA}{Style.BRIGHT}└───> {Style.RESET_ALL}").strip()
        if len(golden_key) != 32:
            print(f"\n{Fore.WHITE}{Style.BRIGHT}Неверный формат токена. Попробуй ещё раз! {Fore.MAGENTA}✧⁠\⁠(⁠>⁠o⁠<⁠)⁠ﾉ⁠✧{Style.RESET_ALL}")
            continue
    ...
    print(f"\n{Fore.MAGENTA}{Style.BRIGHT}┌── {Fore.WHITE}"
          ...
          f"{Fore.MAGENTA}(⁠*⁠´⁠ω`*⁠){Style.RESET_ALL}")
    ...
    while True:
        print(f"\n{Fore.MAGENTA}{Style.BRIGHT}┌── {Fore.WHITE}"
              f"Нужно ли включать Telegram бота? (1 - да / 0 - нет) {Fore.MAGENTA}(Ծ-Ծ){Style.RESET_ALL}")

        tg = input(f"{Fore.MAGENTA}{Style.BRIGHT}└───> {Style.RESET_ALL}").strip()
        if tg not in ["0", "1"]:
            print(f"\n{Fore.WHITE}{Style.BRIGHT}Не понял тебя.. Попробуй ещё раз! {Fore.MAGENTA}(⁠ﾉﾟ⁠0ﾟ⁠)⁠ﾉ{Style.RESET_ALL}")
            continue
        if tg == "1":
            while True:
                print(f"\n{Fore.MAGENTA}{Style.BRIGHT}┌── {Fore.WHITE}Введи токен Telegram-бота "
                      f" {Fore.MAGENTA}(⁠◕⁠દ⁠◕⁠){Style.RESET_ALL}")
                token = input(f"{Fore.MAGENTA}{Style.BRIGHT}└───> {Style.RESET_ALL}").strip()
                if not token:
                    print(f"\n{Fore.WHITE}{Style.BRIGHT}Попробуй ещё раз! {Fore.MAGENTA}⋋⁠✿⁰⁠ ⁠o⁠ ⁠⁰✿⁠⋌{Style.RESET_ALL}")
                    continue
                break

            while True:
                print(f"\n{Fore.MAGENTA}{Style.BRIGHT}┌── {Fore.WHITE}Придумай пароль (его потребует Telegram-бот) "
                      f" {Fore.MAGENTA}(⁠｡⁠♡⁠‿⁠♡⁠｡⁠){Style.RESET_ALL}")
                password = input(f"{Fore.MAGENTA}{Style.BRIGHT}└───> {Style.RESET_ALL}").strip()
                if not password:
                    print(f"\n{Fore.WHITE}{Style.BRIGHT}Попробуй ещё раз! {Fore.MAGENTA}(⁠>⁠0⁠<){Style.RESET_ALL}")
                    continue
                break
    ...
    print(f"\n{Fore.WHITE}{Style.BRIGHT}Готово! Сейчас я сохраню конфиг и завершу программу! "
          f"{Fore.MAGENTA}(⁠≧⁠▽⁠≦⁠){Style.RESET_ALL}")
    print(f"{Fore.WHITE}{Style.BRIGHT}Запусти меня снова и напиши своему Telegram-боту (если ты его включил). "
          f"Все остальное ты сможешь настроить через него. {Fore.MAGENTA}◝⁠(⁠⁰⁠▿⁠⁰⁠)⁠◜{Style.RESET_ALL}")
