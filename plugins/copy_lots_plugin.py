...
def init_commands(celestial: Celestial):
    if not celestial.telegram:
        return
    tg = celestial.telegram
    bot = celestial.telegram.bot
    ...
    def get_current_account(tg_msg: Message) -> FunPayAPI.types.UserProfile:
        ...
        while attempts:
            try:
                profile = celestial.account.get_user(celestial.account.id)
                return profile
            except:
    ...
    def get_lots_info(tg_msg: Message, profile: FunPayAPI.types.UserProfile) -> list[FunPayAPI.types.LotFields]:
        ...
        for i in profile.get_lots():
            ...
            while attempts:
                try:
                    result.append(celestial.account.get_lot_fields(i.id))
        ...
        try:
            ...
            for i in data:
                try:
                    ...
                    create_lot(celestial.account, lot)
                except:
    ...
    celestial.add_telegram_commands(UUID, [
